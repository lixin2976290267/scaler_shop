package com.scaler.auth.service.api.impl;

import com.scaler.auth.mapper.AppInfoMapper;
import com.scaler.auth.mapper.entity.ScalerAppInfo;
import com.scaler.auth.service.api.AuthorizationService;
import com.scaler.auth.utils.Guid;
...
public class AuthorizationServiceImpl extends BaseApiService<JSONObject> implements AuthorizationService {
	...
	public BaseResponse<JSONObject> applyAppInfo(String appName) {
		...
		// 3.添加数据库中
		ScalerAppInfo meiteAppInfo = new ScalerAppInfo(appName, appId, appScrect);
	...
	public BaseResponse<JSONObject> getAccessToken(String appId, String appSecret) {
		...
		ScalerAppInfo meiteAppInfo = appInfoMapper.selectByAppInfo(appId, appSecret);
	...
	public BaseResponse<JSONObject> getAppInfo(String accessToken) {
		...
		// 3.使用appid查询数据库
		ScalerAppInfo meiteAppInfo = appInfoMapper.findByAppInfo(appId);
		if (meiteAppInfo == null) {
			return setResultError("AccessToken  invalid");
		}
		// 4.返回应用机构信息
